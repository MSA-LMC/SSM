...
class DisfaSSM(nn.Module):
    ...
    def __init__(
        ...
        if emotion_count not in {7, 11}:
            raise ValueError(f"Unsupported emotion class count: {emotion_count}")
        if len(input_text2) != 8:
            raise ValueError("DISFA requires eight action-unit descriptions.")
        self.args = args
        ...
        dfer_head_rng_state = torch.get_rng_state()
        ...
        with torch.random.fork_rng(devices=[]):
            torch.set_rng_state(dfer_head_rng_state)
        ...
        self.delta_map1 = nn.Parameter(torch.zeros_like(init_map))
        self.delta_map2 = nn.Parameter(torch.zeros_like(init_map.t()))
    ...
    def _compute_maps(self, eps=1e-8):
        ...
        logits1 = torch.log(self.init_map + eps) + self.delta_map1
        map_au2emo = F.softmax(logits1, dim=1)
        map_au2emo = map_au2emo * self.row_targets_au2emo.view(-1, 1)
        map_au2emo = torch.cat(
            [
                map_au2emo[:neutral_idx],
                self.init_map[neutral_idx : neutral_idx + 1],
        ...
        logits2_non_neutral = (
            torch.log(init_map_t[:, non_neutral_mask] + eps)
        ...
        remaining_targets = self.row_targets_emo2au.view(-1, 1) - neutral_column
        map_emo2au_non_neutral = map_emo2au_non_neutral * remaining_targets

        map_emo2au = torch.zeros_like(init_map_t)
        map_emo2au[:, non_neutral_mask] = map_emo2au_non_neutral
        map_emo2au[
            :,
            neutral_idx : neutral_idx + 1,
        ] = neutral_column
    ...
    def forward(self, x, y):
        ...
        if (
            ...
            raise ValueError("SSM requires matching 16-frame RGB clip shapes.")
        ...
        text_features1 = text_features1 / text_features1.norm(
            dim=-1,
            keepdim=True,
        )
        ...
        text_features2 = text_features2 / text_features2.norm(
            dim=-1,
            keepdim=True,
        )
        ...
        text_features1_z = text_features1 + a * aug_text_features1
        text_features1_z = text_features1_z / text_features1_z.norm(
            dim=-1,
            keepdim=True,
        )
        ...
        text_features2_z = text_features2 + b * aug_text_features2
        text_features2_z = text_features2_z / text_features2_z.norm(
            dim=-1,
            keepdim=True,
        )
        ...
        combined_fer = self.encoder(x_flat.to(self.dtype))
        ...
        dfer_feat = dfer_feat / dfer_feat.norm(
            dim=-1,
            keepdim=True,
        )

        dfer_logits_2 = dfer_feat @ text_features1_z.t() / 0.01
        dfer_logits = dfer_logits_1 + 0.1 * dfer_logits_2

        # Retain one temporally contextualized prediction per AU frame.
        y_flat = y.view(-1, C, H, W)
        combined_au = self.encoder(y_flat.to(self.dtype))
        combined_au = combined_au.reshape(
            B2,
            16,
            512,
        )
        combined_au = self.temporal_net_au(combined_au)
        combined_au = combined_au.reshape(-1, 512)
        au_logits_flat_1 = self.au_head(combined_au)
        combined_au = combined_au / combined_au.norm(
            dim=-1,
            keepdim=True,
        )
        au_logits_flat_2 = combined_au @ text_features2_z.t() / 0.01
        au_logits_flat = au_logits_flat_1 + 0.1 * au_logits_flat_2
