...
class TrueMoE_FFN(nn.Module):
    def __init__(
        ...
        self.shared_act = (
            orig_mlp.act
            if hasattr(orig_mlp, "act")
            else nn.GELU()
        )
        self.shared_fc2 = (
            orig_mlp.c_proj
            if hasattr(orig_mlp, "c_proj")
            else orig_mlp.c_proj
        )
    ...
    def forward(
        ...
        if self.training:
            noise = (
                torch.randn_like(logits_private)
                * self.noise_std
            )
