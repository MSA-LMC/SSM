...
class Bp4dSSM(nn.Module):
    def __init__(
        ...
        if len(input_text2) != 12:
            raise ValueError("BP4D requires 12 action-unit descriptions.")
        self.args = args
    ...
    def _compute_maps(self):
        ...
        map_au2emo = F.softmax(logits1, dim=1) * self.row_targets_au2emo.unsqueeze(1)
        ...
        logits2_non_neutral = (
            torch.log(init_map_emo2au[:, non_neutral_mask] + eps)
        ...
        remaining_targets = (
            self.row_targets_emo2au.unsqueeze(1) - fixed_neutral_col
        ).clamp_min(0.0)
    ...
    def forward(self, x, y):
        ...
        dfer_feat = dfer_feat / dfer_feat.norm(dim=-1, keepdim=True)
