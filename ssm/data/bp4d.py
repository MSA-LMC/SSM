...
class Bp4dSequenceDataset(Dataset):
    ...
    def __init__(
        ...
        if self.complete_evaluation and self.stride != self.clip_len:
            raise ValueError("Complete BP4D evaluation requires stride == clip_len.")

        self.au_ids = (
            list(aus_list) if aus_list is not None else list(self.DEFAULT_AU_IDS)
        )
    ...
    def _resolve_image_path(self, raw_path: str) -> str:
        ...
        return os.path.abspath(os.path.join(self.root, raw_path.lstrip("/\\")))
    ...
    def _extract_sequence_information(
        ...
        Returns:
            ...
            sequence_folder:
                2F23_10
        """
        sequence_folder = os.path.basename(os.path.dirname(image_path))
    ...
    def _parse_line(
            ...
            self.invalid_records.append(
                {
                    ...
                    "reason": (f"Expected {expected_fields} fields, got {len(parts)}."),
                    "line": line,
                }
            )
            return None
        ...
        sequence_information = self._extract_sequence_information(image_path)
            ...
            self.invalid_records.append(
                {
                    "line_number": line_number,
                    "reason": (
                        "Cannot parse sequence folder. Expected a form such as 2F23_10."
                    ),
    ...
    def _remove_duplicate_frames(
        ...
        return [unique_records[index] for index in sorted(unique_records)]
    ...
    def _build_samples(self) -> None:
            ...
            for segment in segments:
                if self.complete_evaluation:
                    ...
                    for start in range(0, len(segment), self.clip_len):
                        ...
                        self.samples.append(padded_records)
                ...
                for start in range(
                    ...
                    clip_records = segment[start : start + self.clip_len]

                    subjects = {record["subject_id"] for record in clip_records}

                    sequence_paths = {
                        record["sequence_path"] for record in clip_records
                    }

                    frame_indices = [record["frame_index"] for record in clip_records]
        ...
        if self.complete_evaluation and len(self.samples) != len(self.eval_metadata):
            raise RuntimeError("BP4D complete-evaluation metadata is inconsistent.")
    ...
    def get_eval_metadata(self, index: int) -> Dict:
    ...
    def pad_complete_evaluation(self, multiple: int) -> int:
        if not self.complete_evaluation:
    ...
    def _print_summary(self) -> None:
        ...
        if self.invalid_records:
            ...
            for item in self.invalid_records[:10]:
                print(f"  line {item.get('line_number')}: {item.get('reason')}")
