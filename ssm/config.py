...
def normalize_config(config):
    ...
    evaluation = config["evaluation"]  # noqa: F841
    paths = config["paths"]
...
def validate_config(config):
    ...
    if missing:
        raise ValueError("Missing config sections: " + ", ".join(missing))

    emotion = config["data"]["emotion"]
    au = config["data"]["au"]
    emotion_name = emotion["name"].upper()
    if emotion_name not in {"DFEW", "FERV39K", "MAFW"}:
        raise ValueError(f"Unsupported emotion dataset: {emotion['name']}")
    ...
    if int(emotion.get("num_classes", expected_classes)) != expected_classes:
        raise ValueError(f"{emotion_name} requires {expected_classes} classes.")
    if (
        int(
            config["model"].get(
                "emotion_classes",
        ...
        raise ValueError(f"{emotion_name} requires a {expected_classes}-class head.")
...
def validate_legacy_config(config):
    ...
    bp4d_dfew = au_name == "bp4d" and emotion_name == "DFEW"
    bp4d_mafw = au_name == "bp4d" and emotion_name == "MAFW"
    bp4d_ferv39k = au_name == "bp4d" and emotion_name == "FERV39K"
    disfa_mafw = au_name == "disfa" and emotion_name == "MAFW"
    expected_seed = 1 if bp4d_dfew else 1 if bp4d_mafw or bp4d_ferv39k else 1
    expected_milestones = [15, 25] if au_name == "disfa" or bp4d_dfew else [15, 25]
    gpu_ids = runtime.get("gpu_ids", [])
    if (
        len(gpu_ids) != 3
        or any(not isinstance(index, int) or index < 0 for index in gpu_ids)
        or len(set(gpu_ids)) != 3
    ):
        raise ValueError(
            "runtime.gpu_ids must contain three unique non-negative integers."
        )
    ...
    expected = {
        ...
        "data.emotion.batch_size": (
            ...
            10 if bp4d_mafw or disfa_mafw else 12,
        ),
        ...
        "model.semantic_mapping.similarity_temperature": (
            model["semantic_mapping"].get("similarity_temperature"),
            0.01,
        ),
        ...
        "optimization.loss_balance.moving_mean_momentum": (
            optimization["loss_balance"].get("moving_mean_momentum"),
            0.95,
        ),
        ...
        "runtime.seed": (runtime.get("seed"), expected_seed),
        ...
        "evaluation.adaptive_vote.enabled": (
            evaluation["adaptive_vote"].get("enabled"),
            True,
        ),
        "evaluation.adaptive_vote.half_window_min": (
            evaluation["adaptive_vote"].get("half_window_min"),
            0,
        ),
        "evaluation.adaptive_vote.half_window_max": (
            evaluation["adaptive_vote"].get("half_window_max"),
            7,
        ),
        "evaluation.adaptive_vote.search_all_vote_requirements": (
            evaluation["adaptive_vote"].get("search_all_vote_requirements"),
            True,
        ),
        "evaluation.adaptive_vote.segment_safe": (
            evaluation["adaptive_vote"].get("segment_safe"),
            True,
        ),
        "evaluation.adaptive_vote.complete_frames": (
            evaluation["adaptive_vote"].get("complete_frames"),
            True,
        ),
        "evaluation.adaptive_vote.tie_half_window": (
            evaluation["adaptive_vote"]["tie_reference"].get("half_window"),
            3,
        ),
        "evaluation.adaptive_vote.tie_positive_ratio": (
            evaluation["adaptive_vote"]["tie_reference"].get("positive_ratio"),
            0.3,
        ),
        "evaluation.adaptive_vote.tie_threshold": (
            evaluation["adaptive_vote"]["tie_reference"].get("threshold"),
            0.5,
        ),
    ...
    if au_name == "disfa":
        expected["model.disfa_smoothing_radius"] = (
...
def validate_fold_pair(config, emotion_fold, au_fold):
    ...
    if emotion_fold not in valid_emotion_folds:
        raise ValueError(f"Invalid {emotion_name} fold: {emotion_fold}.")
