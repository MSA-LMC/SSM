...
from ssm.training.legacy import (
    ...
    preserve_reproducible_rng_state,
...
def build_runtime_args(config):
    ...
    return SimpleNamespace(
        ...
        milestones=[int(value) for value in optimization.get("milestones", [])],
...
def _validate_runtime_device(config, device):
    if device.type != "cuda":
        return
    runtime = config["runtime"]
    if config.get("legacy_compatibility", False) and device.index is not None:
        raise ValueError(
            "Legacy-compatible CUDA runs require the unindexed 'cuda' device."
        )
...
def build_data_loaders(
    ...
    if not emotion_test_list.is_file():
        raise FileNotFoundError(f"Emotion test split not found: {emotion_test_list}")
    ...
    val_loader1 = torch.utils.data.DataLoader(
        ...
        num_workers=args.workers,
        pin_memory=True,
        drop_last=bool(emotion_config.get("drop_last_val", False)),
    )
    if not val_loader1.drop_last:
        val_loader1.au_companion_loader = torch.utils.data.DataLoader(
            test_data1,
            batch_size=args.batch_size,
            shuffle=emotion_config.get("shuffle_val", False),
            num_workers=args.workers,
            pin_memory=True,
            drop_last=True,
        )
    ...
    if not au_train_list.is_file():
        raise FileNotFoundError(f"AU training split not found: {au_train_list}")
    if not au_test_list.is_file():
        raise FileNotFoundError(f"AU test split not found: {au_test_list}")
    ...
    if config["evaluation"]["adaptive_vote"].get("enabled", True):
        if au_name == "bp4d":
            complete_data2 = Bp4dSequenceDataset(
                label_file=str(au_test_list),
                root=str(au_root),
                is_train=False,
                skip_invalid=False,
            ...
            complete_data2 = build_disfa_dataset(
                str(au_test_list),
                is_train=False,
                args=args,
                complete_evaluation=True,
            )
        complete_data2.pad_complete_evaluation(au_batch_size)
        val_loader2.complete_au_loader = torch.utils.data.DataLoader(
            complete_data2,
            batch_size=au_batch_size,
            shuffle=False,
            num_workers=args.workers,
    ...
    if hasattr(val_loader1, "au_companion_loader"):
        loaders["emotion validation AU companion"] = val_loader1.au_companion_loader
    empty = [name for name, loader in loaders.items() if len(loader) == 0]
    if empty:
        raise RuntimeError(
...
def check_data_paths(config, emotion_fold, au_fold):
    ...
    for label, path, expected_type in checks:
        valid = path is not None and (
            path.is_dir() if expected_type == "directory" else path.is_file()
        )
        if not valid:
            invalid.append(f"{label} ({expected_type}): {path}")
    if invalid:
        raise FileNotFoundError(
...
def _output_paths(config, emotion_fold, au_fold, timestamp):
    ...
    run_name = f"{emotion_name}_{au_name}_emotion{emotion_fold}_au{au_fold}_{timestamp}"
...
def _load_checkpoint_file(checkpoint_path, device):
    ...
    for module_name in ("__main__", "main", "main1"):
        ...
        restored_symbols.append((module_name, module, created, existed, previous))
...
def load_checkpoint_state(model, checkpoint_path, device):
    ...
    try:
        ...
        model_has_module = all(key.startswith("module.") for key in model.state_dict())
...
def _append_best_metrics(
    ...
    with open(log_path, "a", encoding="utf-8") as file:
        file.write(f"best F1: {max_f1:.4f} at epoch {best_f1_epoch}\n")
        file.write(f"best AUC: {max_auc:.4f} at epoch {best_auc_epoch}\n")
...
def train_fold_pair(
    ...
    with paths["log"].open("a", encoding="utf-8") as file:
        file.write(f"emotion_fold={emotion_fold}\nau_fold={au_fold}\n")

    best_acc = 0.0
    best_acc_epoch = -1
    best_uar = float("-inf")
    best_uar_epoch = -1
    best_war_at_best_uar = 0.0
    recorder = LegacyCurveRecorder(args.epochs)
    model = build_joint_model(config, args, device)
    # Training randomness must not depend on either task branch's size.
    set_reproducible_seed(args.seed)
    ...
    emotion_count = int(config["data"]["emotion"]["num_classes"])
    max_f1 = 0.0
    best_f1_epoch = -1
    max_auc = 0.0
    best_auc_epoch = -1
    best_au_evaluation = None
    complete_emotion_evaluation = (
        config["evaluation"].get("legacy_validation_pairing") == "complete_emotion"
    )
    au_evaluation_emotion_loader = getattr(
        val_loader1,
        "au_companion_loader",
        val_loader1,
    )
    mixup_config = config["optimization"]["mixup"]
    mixup_alpha = (
        float(mixup_config.get("alpha", 0.4))
        if mixup_config.get("enabled", True)
        else 0.0
    )
    ...
    for epoch in range(0, args.epochs):
        ...
        print(
            "map_au2emo",
            torch.round(map_au2emo[:emotion_count, :au_count] * 10000) / 10000,
        )
        print(
            "map_emo2au",
            torch.round(map_emo2au[:au_count, :emotion_count] * 10000) / 10000,
        )
        ...
        ema.apply_shadow()
        try:
            # Evaluation must not alter next epoch's shuffle, Mixup, or MoE RNG.
            with preserve_reproducible_rng_state():
                emotion_metrics = {}
                val_acc, val_los = validate_joint(
                    ...
                    str(paths["log"]),
                    device,
                    complete_primary=complete_emotion_evaluation,
                    emotion_metrics=emotion_metrics,
                )
                val_uar = emotion_metrics["uar"]
                val_war = emotion_metrics["war"]
                val_acc = float(val_acc)
                is_best_acc = epoch == 0 or val_acc > best_acc
                if is_best_acc:
                    best_acc = val_acc
                    best_acc_epoch = epoch
                is_best = epoch == 0 or val_uar > best_uar
                if is_best:
                    best_uar = val_uar
                    best_uar_epoch = epoch
                    best_war_at_best_uar = val_war
                save_checkpoint(
                    {
                        "epoch": epoch + 1,
                ...
                au_evaluation, f1_mean, auc_mean = evaluate_action_units(
                    au_evaluation_emotion_loader,
                    val_loader2,
                    model,
                    device,
                    au_name,
                ...
                if epoch == 0 or f1_mean > max_f1:
                    max_f1 = f1_mean
                    best_f1_epoch = epoch
                    best_au_evaluation = au_evaluation
        ...
        print(
            "The best validation UAR: {:.3f} at epoch {}".format(
                best_uar,
                best_uar_epoch,
            )
        )
        print("An epoch time: {:.2f}s".format(epoch_time))
        print(f"Current epoch {epoch} - F1: {f1_mean:.4f}, AUC: {auc_mean:.4f}")
    ...
    # Final expression metrics use the configured validation checkpoint.
    ...
    uar, war = evaluate_emotions(
        ...
        model,
        device,
        get_emotion_labels(config["data"]["emotion"]["name"]),
    ...
    summary = {
        ...
        "best_emotion_accuracy_epoch": best_acc_epoch,
        "best_emotion_uar": float(best_uar),
        "best_emotion_uar_epoch": best_uar_epoch,
        "best_emotion_war_at_best_uar": float(best_war_at_best_uar),
        "emotion_checkpoint_selection_metric": "emotion_validation_uar",
        "best_au_f1": float(max_f1),
        "best_au_f1_epoch": best_f1_epoch,
        "best_au_auc": float(max_auc),
        "best_au_auc_epoch": best_auc_epoch,
        "best_au_evaluation": best_au_evaluation,
        "uar": float(uar),
        "war": float(war),
