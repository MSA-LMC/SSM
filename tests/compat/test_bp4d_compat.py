...
def test_mixup_uses_numpy_then_cpu_torch_rng_on_flattened_frames():
    frames = torch.arange(8 * 3, dtype=torch.float32).reshape(8, 3)
    targets = torch.arange(8 * 2, dtype=torch.float32).reshape(8, 2)

    np.random.seed(2026)
    torch.manual_seed(490)
    expected_lam = np.random.beta(0.4, 0.4)
    expected_index = torch.randperm(frames.size(0))
    expected_frames = (
        expected_lam * frames
        + (1 - expected_lam) * frames[expected_index, :]
    )
...
def test_ema_update_apply_and_restore_match_the_legacy_equations():
    ...
    expected_shadow = (
        torch.tensor([[2.0]]) * 0.99
        + torch.tensor([[6.0]]) * 0.01
    )
    ema.update()
...
def test_active_group_augmentations_preserve_rng_call_order():
    ...
    images = [
        Image.fromarray(np.roll(pixels, shift, axis=1))
        for shift in (0, 7)
    ]
    ...
    random.seed(seed)
    actual_crops = GroupRandomSizedCrop(64)(
    ...
    expected_next = reference_rng.random()
    random.seed(seed)
    flipped = GroupRandomHorizontalFlip()(
        [image.copy() for image in images]
    )
    actual_next = random.random()
    if should_flip:
    ...
    expected_next = reference_rng.random()
    random.seed(seed)
    jittered = ColorJitter(brightness=0.5)(
        [image.copy() for image in images]
    )
    actual_next = random.random()
...
def test_mafw_training_and_evaluation_normalization_remain_compatible():
    ...
    calls = {
        name: _branch_calls(branch, name)
...
def test_joint_step_locks_flattened_mixup_and_four_loss_equation():
    source = _compact_function_source(
    ...
    required = [
        ...
        "loss2=lam*criterion2(au_output,targets_a)+(1-lam)"
        "*criterion2(au_output,targets_b)",
        "loss3=criterion1(dfer_output_pro,dfer_labels)",
        "loss4=lam*criterion2(au_output_pro,targets_a)+(1-lam)"
        "*criterion2(au_output_pro,targets_b)",
        "momentum=0.95",
        "eps=1e-8",
        "p=epoch/30",
        "a=2/(1+np.exp(-10*p))-1",
        "loss1_scaled+2.00*loss2_scaled"
...
def test_validation_and_evaluations_zip_the_two_loaders():
    checks = [
        (LEGACY_TRAINING_PATH, "validate_joint"),
        (METRICS_PATH, "evaluate_action_units"),
...
def test_majority_vote_threshold_search_and_war_denominator_are_legacy():
    metrics_tree = _tree(METRICS_PATH)
    vote_node = next(
        node
        for node in metrics_tree.body
        if isinstance(node, ast.FunctionDef)
        and node.name == "majority_voting"
    )
    ...
    assert "threshold=0.5" in au_source
    assert "foriinrange(1,100):" in au_source
    assert "th=i*0.01" in au_source
    assert "majority_voting(y_thr,K=K,p=p)" in au_source
    assert "np.argmax(np.mean(f1_score_arr,axis=1))" in au_source
...
def test_runner_builds_emotion_loaders_before_bp4d_loaders():
    ...
    assert source.count("shuffle=False") == 2
