...
def test_release_configs_validate_and_select_the_expected_pair():
    for filename, expected in EXPECTED_CONFIGS.items():
        ...
        if expected[1] == "BP4D":
            assert au_paths["train_list"].endswith(
                "train_{au_fold}f_data.list"
            )
            assert au_paths["test_list"].endswith(
                "test_{au_fold}f_data.list"
            )
        else:
            assert au_paths["train_list"].endswith(
                "DISFA_train{au_fold}.json"
            )
            assert au_paths["test_list"].endswith(
                "DISFA_test{au_fold}.json"
            )


def test_effective_legacy_aliases_cannot_bypass_reproduction_settings():
    source = load_config(
        REPOSITORY_ROOT / "configs" / "bp4d_dfew.json"
    )
    mutations = [
        ...
        (
            ...
            ("evaluation", "voting_radius", 9),
        ),
        (
            "evaluation.voting_positive_ratio",
            ("evaluation", "voting_positive_ratio", 0.9),
        ),
        (
...
def test_only_official_expression_and_au_fold_ids_are_accepted():
    ferv = load_config(
        REPOSITORY_ROOT / "configs" / "bp4d_ferv39k.json"
    )
    with pytest.raises(ValueError, match="FERV39K fold"):
        validate_fold_pair(ferv, 2, 1)

    mafw = load_config(
        REPOSITORY_ROOT / "configs" / "disfa_mafw.json"
    )
...
def test_all_fold_order_matches_the_original_nested_loops():
    ...
    assert all_fold_pairs("FERV39K") == [
        {"emotion_fold": 1, "au_fold": au_fold}
        for au_fold in [1, 2, 3]
    ]
