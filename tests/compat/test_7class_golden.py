...
def test_seven_class_model_schema_parameter_order_and_outputs_are_frozen():
    cases = {
        "bp4d": {
            "schema": (
                "5812b6259177ed0ca7ef7fd046d6e11a2e50dbc70b23df9dbd56a3ae51275f1b"
            ),
            "parameters": (
                "212430f47f71dd85eafb0a20b90df815243eea8dbbcb807f0d752bef556350d7"
            ),
            "shapes": (
                (1, 7),
                (1, 7),
        ...
        "disfa": {
            "schema": (
                "b68cf5a9b94b8b22f7ec87bb5df9f5a3c0f40f00c5d5fef52ca7812b94e5a997"
            ),
            "parameters": (
                "146ed435d872df52f122cdf2c16a0fd28a5a47483cb69abdae888862749b04b0"
            ),
            "shapes": (
                (1, 7),
                (1, 7),
    ...
    for au_dataset, expected in cases.items():
        ...
        assert _parameter_order_digest(model) == expected["parameters"]
        assert model.dfer_head.out_features == 7

        model.eval()
        with torch.no_grad():
            outputs = model(x, y)
        assert tuple(tuple(output.shape) for output in outputs) == (expected["shapes"])
