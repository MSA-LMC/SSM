...
def main():
    ...
    if cli.all_folds and (
        cli.emotion_fold is not None or cli.au_fold is not None
    ):
        raise ValueError(
            "--all-folds cannot be combined with fold overrides."
        )
    if cli.all_folds:
        ...
        if cli.emotion_fold is None or cli.au_fold is None:
            raise ValueError(
                "--emotion-fold and --au-fold must be provided together."
            )
