...
experiment split manifests migrated from the legacy research code. Only
machine-specific path prefixes were removed: record order, frame counts, and
labels are preserved. Obtain each dataset under its own license and terms of
use.
...
The release uses CLIP normalization with mean `[0.481, 0.457, 0.408]` and standard deviation `[0.268, 0.261, 0.275]`. DFEW training applies the legacy brightness jitter, group random crop, and group horizontal flip defined in the config and loader.

## FERV39K

FERV39K uses the same three-field manifest schema and seven-class label order as DFEW. Paths in the included manifests are relative to `data/FERV39K/frames`. The training transform preserves the legacy 4-degree random rotation, group random crop, group horizontal flip, and CLIP normalization. FERV39K has one expression train/test split; `experiment.au_fold` selects the BP4D or DISFA fold.
...
Training uses the legacy group random crop, group horizontal flip, and ImageNet normalization. Evaluation uses resize and CLIP normalization.
